...
valid_y_t = torch.from_numpy(valid_y)
...
train_data = TensorDataset(train_x_t, train_y_t)
valid_data = TensorDataset(valid_x_t, valid_y_t)
test_data = TensorDataset(test_x_t, test_y_t)
...
class SentimentLSTM(nn.Module):
    ...
    def forward (self, input_words):
        ...
        embedded_words = self.embedding(input_words.long())    # (batch_size, seq_length, n_embed)
...
if USE_PICKLED_MODEL:
    import pickle
    fileo = open(r'./net.pkl', 'rb')
    net = pickle.load(fileo)
    fileo.close()
else:
    for epoch in range(n_epochs):
        ...
        for inputs, labels in train_loader:
            step += 1
            print('STEP', step, 'of', len(train_loader.dataset.tensors[0])/train_loader.batch_size)
...
def predict(net, review, seq_length = 200):
    ...
    words = preprocess_string(review)
    ...
    pred2 = int(pred[0])
    msg = "This is a positive review." if pred2 == 0 else "This is a negative review."
    print(msg)
    return msg
